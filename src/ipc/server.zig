...
pub const IpcServer = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, controller: *BrightnessController, logger: *Logger) !Self {
        const raw_socket = linux.socket(linux.AF.UNIX, linux.SOCK.STREAM | linux.SOCK.CLOEXEC, 0);
        if (raw_socket < 0) {
            logger.err("创建 socket 失败: {}", .{raw_socket}) catch {};
        ...
        _ = std.fs.deleteFileAbsolute("/tmp/beelight.sock") catch |err| switch (err) {
            error.FileNotFound => {},
                ...
                logger.err("删除旧 socket 文件失败: {}", .{err}) catch {};
                return err;
            },
        };
        ...
        if (rc != 0) {
            logger.err("绑定 socket 失败: {}", .{rc}) catch {};
            return error.BindFailed;
        }

        const rc2 = linux.listen(socket, 128);
        if (rc2 != 0) {
            logger.err("监听 socket 失败: {}", .{rc2}) catch {};
        ...
        if (rc3 < 0) {
            logger.err("设置 socket 权限失败: {}", .{rc3}) catch {};
            return error.ChmodFailed;
        }

        logger.info("IPC 服务器初始化成功", .{}) catch {};
    ...
    fn handleClient(self: *Self, client_socket: i32) !void {
        ...
        if (rc == 0) {
            self.logger.info("客户端连接 - PID: {}, UID: {}, GID: {}", .{ cred.pid, cred.uid, cred.gid }) catch {};
        }

        var buf: [4096]u8 = undefined;
        const recv_len = linux.recvfrom(client_socket, &buf, buf.len, 0, null, null);
        if (recv_len < 0) {
            self.logger.err("接收数据失败: {}", .{recv_len}) catch {};
            return error.RecvFailed;
        }
        if (recv_len == 0) return;

        const parsed = std.json.parseFromSlice(Command, self.allocator, buf[0..@intCast(recv_len)], .{}) catch {
            self.logger.err("解析命令失败，客户端 PID: {}", .{if (rc == 0) cred.pid else 0}) catch {};
            try self.sendError(client_socket, "无效的命令格式");
            return;
        };
        defer parsed.deinit();

        self.logger.info("收到命令 - PID: {}, 命令: {}", .{ if (rc == 0) cred.pid else 0, parsed.value }) catch {};
    ...
    fn handleCommand(self: *Self, client_socket: i32, parsed_command: Command) !void {
        switch (parsed_command) {
            .set_brightness => |value| {
                self.logger.info("收到设置亮度命令: {d}%", .{value}) catch {};
                ...
                self.logger.info("亮度已设置: {d}% -> {d} (原始值)", .{ value, raw_value }) catch {};
            ...
            .get_brightness => {
                self.logger.info("收到获取亮度命令", .{}) catch {};
                ...
                self.logger.info("当前亮度: {d} (原始值) -> {d}%", .{ raw_brightness, percentage }) catch {};
                try self.sendResponse(client_socket, .{
                    .success = true,
                    .message = "获取亮度成功",
            ...
            .toggle_auto => {
                ...
                self.logger.info("收到切换自动模式命令，当前状态: {}", .{old_mode}) catch {};
                try self.controller.setAutoMode(!old_mode);
                self.logger.info("自动模式已切换: {} -> {}", .{ old_mode, self.controller.auto_mode }) catch {};
                try self.sendResponse(client_socket, .{
                    .success = true,
            ...
            .show_stats => {
                self.logger.info("收到获取统计信息命令", .{}) catch {};
                ...
                self.logger.info("系统状态 - 环境光: {d}, 亮度: {d} ({d}%), 自动模式: {}", .{ ambient, raw_brightness, percentage, self.controller.auto_mode }) catch {};
                try self.sendResponse(client_socket, .{
                    .success = true,
                    .message = "获取统计信息成功",
                    .data = .{ .stats = .{
    ...
    fn sendResponse(self: *Self, client_socket: i32, response: Response) !void {
        ...
        if (rc < 0) {
            self.logger.err("发送响应失败: {}", .{rc}) catch {};
            return error.SendFailed;
        }
        self.logger.debug("已发送响应: {s}", .{msg.items}) catch {};
    }

    fn sendError(self: *Self, client_socket: i32, message: []const u8) !void {
        self.logger.err("发送错误响应: {s}", .{message}) catch {};
        try self.sendResponse(client_socket, .{
            .success = false,
            .message = message,
            .data = null,
